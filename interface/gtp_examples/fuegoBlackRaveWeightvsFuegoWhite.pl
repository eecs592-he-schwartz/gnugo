...
my @program = ();
my @cur_id = ();
my $random_int32 = int(rand(2**32)) - 2**31;

my $Aprg_in = new FileHandle;
my $Aprg_out = new FileHandle;
$program[0] = 'fuego --config configFilesOptions/configfileinalrave4500 --quiet';
...
$program[1] = 'fuego --config configfile --quiet';
...
sub  control
{
	...
	if ( $msg =~ /PASS/ ) {
	    ...
	    $gn = 'PASS';
            ...
            snd(0, "$cur_id[1] savesgf $ARGV[0]BLACKRESIGN${random_int32}.sgf");
	    die
	...
	if ( $msg =~ /PASS/ ) {
	    ...
	    $gn = 'PASS';
            ...
            snd(1, "$cur_id[0] savesgf $ARGV[0]WHITERESIGN${random_int32}.sgf");
	    die
	    #snd(1, "$cur_id[0] play white $gn");
            $state = 'gameover';
            return;
        }
	else {
	...
	ttScore();
        ...
        snd(0, "$cur_id[0] savesgf GAMEOVER${random_int32}.sgf");
        $state = 'score black';
        return;
    }
	...
	print "black final_score:\n";
        ...
        snd(1, "$cur_id[1] savesgf GAMEOVER${random_int32}.sgf");
        $state = 'score white';
        return;
    }
